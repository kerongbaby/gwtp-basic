...
public abstract class Auth {
  ...
  public String toUrl(AuthRequest req) {
	  return req.toUrl(urlCodex) + "&redirect_uri=" + urlCodex.encode(oauthWindowUrl);
  }
  ...
  public static native void export() /*-{
    ...
    $wnd.oauth2.login = $entry(function(req, success, failure) {
      @com.risetek.demo.client.oauth.Auth::nativeLogin(*)(req, success, failure);
    });

    $wnd.oauth2.expiresIn = $entry(function(req) {
      return @com.risetek.demo.client.oauth.Auth::nativeExpiresIn(*)(req);
    });

    $wnd.oauth2.clearAllTokens = $entry(function() {
      @com.risetek.demo.client.oauth.Auth::nativeClearAllTokens()();
    });
  }-*/;
